...
export class BlockReminderJob {
  private readonly logger = new Logger(BlockReminderJob.name);
  ...
  async processBlockReminders(): Promise<void> {
    this.logProvider.info({ message: '⏰ Block reminder job started', context: 'BlockReminderJob.processBlockReminders', params: {} });
    ...
    try {
      ...
      for (const block of blocks) {
        const daysSinceBlock = Math.floor((Date.now() - new Date(block.blocked_at).getTime()) / DAY_MS) + 1;
        ...
        if (daysSinceBlock >= 5) {
          ...
          this.logProvider.warn({
            ...
            context: 'BlockReminderJob.processBlockReminders',
            params: { blockId: block.id, userId: block.user_id, reason: block.reason, days: daysSinceBlock },
          });
    ...
    this.logProvider.info({ message: '⏰ Block reminder job finished', context: 'BlockReminderJob.processBlockReminders', params: {} });
  }

  @Cron(CronExpression.EVERY_DAY_AT_10AM)
  async processExpiredBlocks(): Promise<void> {
    this.logProvider.info({ message: '⏰ Expired block cleanup started', context: 'BlockReminderJob.processExpiredBlocks', params: {} });
