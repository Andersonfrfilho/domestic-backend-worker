...
export class ProviderApprovalHandler {
  ...
  async handleApproved(event: ProviderApprovalEvent): Promise<void> {
    this.logger.info({
      ...
      params: { provider_id: event.provider_id, user_id: event.user_id },
    });
    ...
    await this.notificationHandler.persist({
      ...
      metadata: {
        ...
        entity_id: event.provider_id,
        entity_type: 'provider',
      },
    });
    this.publishNotifications(event, 'APPROVED');
    this.logger.info({
      message: 'Provider approval processed successfully',
      context: this.logContext,
  ...
  async handleRejected(event: ProviderApprovalEvent): Promise<void> {
    ...
    this.logger.info({
      message: 'Provider rejection processed successfully',
      context: this.logContext,
      params: { provider_id: event.provider_id },
    });
  }
  ...
  private async updateVerificationStatus(
    ...
    if (verification.status === status) {
      this.logger.warn({
        message: `[idempotent] Verification already ${status} — skipping`,
        context: this.logContext,
        params: { providerId },
      });
      return;
    ...
    this.amqp
      ...
      .catch((err) =>
        this.logger.error({
          ...
          context: this.logContext,
          params: { provider_id: event.provider_id, error: err?.message },
        }),
      );
